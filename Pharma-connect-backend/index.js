...
app.post('/api/transfer', async (req, res) => {
  ...
  try {
    if (secureMode) {
      // Secure transfer without transactions
...
app.get('/api/transactions/:username', async (req, res) => {
  ...
  try {
    if (secureMode === 'true') {
      // Secure version - only return transactions where user is sender or recipient
      ...
      // For vulnerable mode, we'll use a more permissive query
      // This simulates a NoSQL injection vulnerability
      const transactions = await Transaction.find({
        $or: [
          { fromUser: { $regex: new RegExp(username, 'i') } },
